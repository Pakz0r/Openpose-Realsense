...
namespace SA
{
	...
	public partial class FullBodyIK
	{
		...
		static Bone PrepareBone(Bone bone)
		{
			return (bone != null && bone.TransformIsAlive) ? bone : null;
		}

		static Bone[] PrepareBones(Bone leftBone, Bone rightBone)
		{
		...
		static bool ComputeEyesRange(ref Vector3 eyesDir, float rangeTheta)
		{
			if (rangeTheta >= -IKEpsilon)
			{ // range
				if (eyesDir.z < 0.0f)
				{
					eyesDir.z = -eyesDir.z;
		...
		public static string GetAvatarName(Transform rootTransform)
		{
