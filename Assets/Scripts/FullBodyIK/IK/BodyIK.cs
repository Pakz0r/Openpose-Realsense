...
namespace SA
{
	public partial class FullBodyIK
	{
		public class BodyIK
		{
			...
			void Prepare(FullBodyIK fullBodyIK)
			{
				if (_spineBones != null)
				{
					int spineLength = _spineBones.Length;
					...
					if (_spineBones != null)
					{
						if ((_nearArmBones != null || _legBones != null) && _neckBone != null && _neckBone.TransformIsAlive)
						{
							...
							for (int i = 0; i < spineLength - 1; ++i)
							{
								...
								if (!SAFBIKComputeBasisFromXYLockY(out Matrix3x3 prevToCenterArmBasis, ref armDirX, ref dirY0) ||
			...
			public bool Solve()
			{
				bool isEffectorEnabled = IsEffectorEnabled();
			...
			bool UpperSolve()
			{
				...
				{
					PresolveBaseCenterLegTransform(out baseCenterLegPos, out Matrix3x3 centerLegBasis);
				...
				Vector3 centerArmDirX = LerpDir(ref currentCenterArmDir, ref targetCenterArmDir, upperLerpDir1Rate);
				Vector3 centerArmDirX2 = LerpDir(ref currentCenterArmDir, ref targetCenterArmDir, upperLerpDir2Rate);
				...
				if (_settings.bodyIK.upperDirXLimitEnabled)
				{
					...
					{
						if (!FitToPlaneDir(ref centerArmDirX, centerArmDirY) ||
							!FitToPlaneDir(ref centerArmDirX2, centerArmDirY2))
						{
							return false;
						}
					}
					...
					{
						if (!LimitToPlaneDirY(ref centerArmDirX, centerArmDirY, _internalValues.bodyIK.upperDirXLimitThetaY.sin) ||
							!LimitToPlaneDirY(ref centerArmDirX2, centerArmDirY2, _internalValues.bodyIK.upperDirXLimitThetaY.sin))
						{
							return false;
						}
					}
					...
					{ // Quaternion lerp.
						...
						if (fromToXAng > spineLimitAngleX)
						{
							...
							if (SAFBIKVecNormalize(ref axisDir))
							{
								...
								SAFBIKMatSetRot(out Matrix3x3 rotateBasis, ref q);
				...
				if (eyesRate > IKEpsilon)
				{
					...
					if (SAFBIKComputeBasisFromXYLockY(out Matrix3x3 toBasis, ref centerArmDirX2, ref centerArmDirY2))
					{
						SAFBIKMatMult(out Matrix3x3 toBasisGlobal, ref toBasis, ref _centerLegToArmBasisInv);

						Matrix3x3 fromBasis = toBasis;
						SAFBIKMatMultRet0(ref toBasis, ref _centerLegToArmBoneToBaseBasis);

						SAFBIKMatMultVecPreSubAdd(out Vector3 eyePos, ref toBasisGlobal, ref _defaultCenterEyePos, ref _defaultCenterLegPos, ref presolvedCenterLegPos2);
...

						{
							...
							{
								LimitXY(ref eyeDir, upperEyesXLimit, upperEyesXLimit, upperEyesYDownLimit, upperEyesYUpLimit);
							}
				...
				{
					if (SAFBIKComputeBasisFromXYLockY(out Matrix3x3 toBasis, ref centerArmDirX, ref centerArmDirY))
					{
						...
						if (_internalValues.animatorEnabled || _internalValues.resetTransforms)
						{
							...
							if (continuousSolverEnabled && stableCenterLegRate > IKEpsilon)
							{
								...
								{
									SAFBIKMatFastLerp(out Matrix3x3 tempBasis, ref toBasis, ref presolveCenterLegBasis, stableCenterLegRate);
									toBasis = tempBasis;
								}
							}
							...
							if (SAFBIKVecNormalize(ref currentDirY) && SAFBIKComputeBasisFromXYLockY(out Matrix3x3 fromBasis, ref currentDirX, ref currentDirY))
							{
								SAFBIKMatMultInv1(out rotateBasis, ref toBasis, ref fromBasis);
							}
						}
						else
						{ // for continuousSolverEnabled.(Base on centerLegBasis)
...

				{
					...
					for (int i = 0; i != spineLength; ++i)
					{
						...
						if (i + 1 == spineLength)
						{
							...
							SAFBIKComputeBasisFromXYLockY(out Matrix3x3 toBasis, ref targetDirX, ref targetDirY);
							SAFBIKComputeBasisFromXYLockY(out Matrix3x3 fromBasis, ref currentDirX, ref currentDirY);
							SAFBIKMatMultInv1(out Matrix3x3 rotateBasis, ref toBasis, ref fromBasis);
						...
						else
						{
							...
							if (!SAFBIKComputeBasisFromXYLockY(out Matrix3x3 prevToNearArmBasis, ref currentDirX, ref targetDirY) ||
								!SAFBIKComputeBasisFromXYLockY(out Matrix3x3 origToNearArmBasis, ref currentDirX, ref targetDirY2))
							{
								continue;
							}
							...
							{ // Exclude spineU
								...
								if (!SAFBIKVecNormalize(ref targetDirY))
								{ // Failsafe.
			...
			void PresolveBaseCenterLegTransform(out Vector3 centerLegPos, out Matrix3x3 centerLegBasis)
			{
				...
				SAFBIKMatMultVecPreSubAdd(out Vector3 legPos0, ref centerLegBasis, ref _legBones[0]._defaultPosition, ref _defaultCenterLegPos, ref centerLegPos);
				SAFBIKMatMultVecPreSubAdd(out Vector3 legPos1, ref centerLegBasis, ref _legBones[1]._defaultPosition, ref _defaultCenterLegPos, ref centerLegPos);
			...
			void SolveUpperTransform(int origIndex, ref Matrix3x3 transformBasis)
			{
				var temp = _solverInternal;
			...
			bool PresolveUpperTranslate2(out Vector3 translate, ref CachedRate01 translateRate, ref CachedRate01 stableRate, ref Vector3 stableCenterLegPos)
			{
			...
			void SolveUpperTranslate2(ref CachedRate01 translateRate, ref CachedRate01 stableRate, ref Vector3 stableCenterLegPos)
			{
				if (PresolveUpperTranslate2(out Vector3 translate, ref translateRate, ref stableRate, ref stableCenterLegPos))
				{
					var temp = _solverInternal;
					Assert(temp != null);
			...
			void SolveLower(bool firstPass)
			{
				var temp = _solverInternal;
				var cache = _solverCaches;
				Assert(temp != null);
				...
				if (temp.PrepareLowerRotation(0))
				{
					...
					if (temp.SolveLowerRotation(0, out Quaternion origLowerRotation))
					{
				...
				else
				{
					...
					{
						if (temp.SolveLowerTranslate(out Vector3 origLowerTranslate))
						{
			...
			void ComputeWorldTransform()
			{
				...
				if (_hipsBone != null && _hipsBone.TransformIsAlive && temp.spinePos != null && temp.spinePos.Length > 0 && _neckBone != null && _neckBone.TransformIsAlive)
				{
					...
					{
						SAFBIKMatMult(out Matrix3x3 tempBasis, ref boneBasis, ref _centerLegBoneBasisInv);

						hipsToSpineDirX = boneBasis.column0; // Counts as baseBasis.

						SAFBIKMatMultGetRot(out Quaternion worldRotation, ref tempBasis, ref _hipsBone._defaultBasis);
						...
						if (_hipsBone.IsWritebackWorldPosition)
						{
							...
							SAFBIKMatMultVecAdd(out Vector3 worldPosition, ref tempBasis, ref inv_defaultLocalTranslate, ref temp.spinePos[0]);
					...
					for (int i = 0; i != spineLength; ++i)
					{
						...
						if (SAFBIKVecNormalize(ref dirY) && SAFBIKComputeBasisFromXYLockY(out boneBasis, ref dirX, ref dirY))
						{
							...
							SAFBIKMatMultGetRot(out Quaternion worldRotation, ref boneBasis, ref _spineBones[i]._boneToWorldBasis);
					...
					if (_shoulderBones != null)
					{
						for (int i = 0; i != 2; ++i)
						{
							...
							if (SAFBIKVecNormalize3(ref xDir, ref yDir, ref zDir))
							{
								...
								SAFBIKMatMultGetRot(out Quaternion worldRotation, ref boneBasis, ref _shoulderBones[i]._boneToWorldBasis);
			...
			bool PrepareSolverInternal()
			{
				...
				for (int i = 0; i != 2; ++i)
				{
					...
					{
						fullArmPull = ConcatPull(fullArmPull, _solverCaches.elbowPull[i]);
					}
					if (fullArmPull == 0.0f)
					{ // Optimized for function calls.
					...
					{
						fullArmPull = ConcatPull(fullArmPull, _solverCaches.wristPull[i]);
					}
					...
					{
						limbArmPull = ConcatPull(limbArmPull, _solverCaches.wristPull[i]);
					}
					...
					{
						legPull = ConcatPull(legPull, _solverCaches.footPull[i]);
					}
			...
			void PresolveHips()
			{
				...
				if (rotationEnabled)
				{
					...
					SAFBIKMatGetRot(out Quaternion centerLegRotationFrom, ref centerLegBasis);
				...
				if (positionEnabled)
				{
					...
					SAFBIKMatMultVecPreSubAdd(out Vector3 centerLegPos, ref centerLegBasis, ref _defaultCenterLegPos, ref _hipsEffector._defaultPosition, ref hipsEffectorWorldPosition);
			...
			void ResetTransforms()
			{
				...
				GetBaseCenterLegTransform(out Vector3 centerLegPos, out Matrix3x3 centerLegBasis);
				ResetCenterLegTransform(ref centerLegPos, ref centerLegBasis);
			}

			void GetBaseCenterLegTransform(out Vector3 centerLegPos, out Matrix3x3 centerLegBasis)
			{
			...
			void ResetCenterLegTransform(ref Vector3 centerLegPos, ref Matrix3x3 centerLegBasis)
			{
				var temp = _solverInternal;
				Assert(temp != null);
			...
			void ResetShoulderTransform()
			{
				...
				if (SAFBIKVecNormalize(ref dirY) && SAFBIKComputeBasisFromXYLockY(out Matrix3x3 boneBasis, ref dirX, ref dirY))
				{
					SAFBIKMatMult(out Matrix3x3 tempBasis, ref boneBasis, ref _spineUBone._localAxisBasisInv);
			...
			class SolverInternal
			{
				public class Limb
				{
					...
					public void ClearEnvTargetBeginPos()
					{
						for (int i = 0; i < 2; ++i)
						{
							ClearEnvTargetBeginPos(i, ref beginPos[i]);
						}
					}
					...
					public void ClearEnvTargetBeginPos(int i, ref Vector3 beginPos)
					{
					...
					static bool SolveTargetBeginPos(ref Vector3 targetBeginPos, ref Vector3 targetEndPos, ref FastLength targetBeginToEndLength, float endPull)
					{
				...
				public void SetCenterArmPos(ref Vector3 centerArmPos)
				{
					_isDirtyCenterArmPos = false;
					_centerArmPos = centerArmPos;
				}

				public void SetCenterLegPos(ref Vector3 centerLegPos)
				{
					_isDirtyCenterLegPos = false;
				...
				struct UpperSolverPreArmsTemp
				{
					...
					{
						UpperSolverPreArmsTemp r = new UpperSolverPreArmsTemp();
				...
				struct UpperSolverArmsTemp
				{
					...
					{
						UpperSolverArmsTemp r = new UpperSolverArmsTemp();
						r.shoulderPos = new Vector3[2];
						r.armPos = new Vector3[2];
						r.nearArmPos = null; // shoulderPos / armPos
						r.shoulderEnabled = false;
				...
				struct UpperSolverTemp
				{
					...
					{
						UpperSolverTemp r = new UpperSolverTemp();
				...
				UpperSolverArmsTemp[] _upperSolverArmsTemps = new UpperSolverArmsTemp[2] { UpperSolverArmsTemp.Alloc(), UpperSolverArmsTemp.Alloc() };
				...
				public bool UpperSolve()
				{
					...
					if (headMovingfixRate < 1.0f - IKEpsilon || bodyMovingfixRate < 1.0f - IKEpsilon)
					{
						if (headPull > IKEpsilon || neckPull > IKEpsilon)
						{
							if (headMovingfixRate < 1.0f - IKEpsilon && headPull > IKEpsilon)
							{
								...
								if (KeepMaxLength(ref tempNeckPos, ref _upperSolverTemp.targetHeadPos, _solverCaches.neckToHeadLength))
								{ // Not KeepLength
					...
					if ((enabled0 && enabled1) || neckHeadPull > IKEpsilon)
					{
						...
						targetCenterArmDir = LerpDir(ref _upperSolverArmsTemps[0].centerArmDir, ref _upperSolverArmsTemps[1].centerArmDir, limbArmRate);
					}
					else
					{
				...
				public bool ShoulderResolve()
				{
					...
					for (int i = 0; i != 2; ++i)
					{
						if (_tempElbowPosEnabled[i])
						{
							ComputeThetaAxis(ref _tempArmToElbowDefaultDir[i], ref _tempArmToElbowDir[i], out float theta, out Vector3 axis);
							...
							else
							{
								...
								SAFBIKMatMultVec(out Vector3 resultDir, ref m, ref tempDir);
				...
				public bool PrepareLowerRotation(int origIndex)
				{
					...
					for (int i = 0; i < 2; ++i)
					{
						...
						r |= PrepareLimbRotation(this.legs, i, origIndex, ref this.legs.beginPos[i]);
					}
					return r;
				}

				public bool PrepareLimbRotation(Limb limb, int i, int origIndex, ref Vector3 beginPos)
				{
					Assert(i < 2);
				...
				{
					return SolveLimbRotation(this.legs, origIndex, out origRotation);
				}

				bool SolveLimbRotation(Limb limb, int origIndex, out Quaternion origRotation)
				{
					...
					if (pullLength == 1)
					{
						...
						origRotation = GetRotation(ref this.origAxis[i0], this.origTheta[i0], this.origFeedbackRate[i0] * lerpRate);
						return true;
					}

					// Fix for rotate 180 degrees or more.( half rotation in GetRotation & double rotation in origRotation * origRotation. )
					Quaternion origRotation0 = GetRotation(ref this.origAxis[0], this.origTheta[0], this.origFeedbackRate[0] * 0.5f);
					Quaternion origRotation1 = GetRotation(ref this.origAxis[1], this.origTheta[1], this.origFeedbackRate[1] * 0.5f);
				...
				public bool PrepareLowerTranslate()
				{
					...
					{
						this.legs.SolveTargetBeginPos(i);
						r |= PrepareLimbTranslate(this.legs, i, ref this.legs.beginPos[i]);
					}
					return r;
				}

				bool PrepareLimbTranslate(Limb limb, int i, ref Vector3 beginPos)
				{
				...
				{
					return SolveLimbTranslate(this.legs, out translate);
				}

				bool SolveLimbTranslate(Limb limb, out Vector3 origTranslate)
				{
				...
				{
					LimbTranslateBeginOnly(this.legs, ref origTranslate);
					_centerLegPos += origTranslate; // Optimized.
				}

				void LimbTranslateBeginOnly(Limb limb, ref Vector3 origTranslate)
				{
					for (int i = 0; i < 2; ++i)
					{
					...
					if (!IsFuzzy(ref armPos[i], ref destArmPos))
					{
						...
						if (SAFBIKVecNormalize(ref dirX))
						{
							if (settings.bodyIK.shoulderLimitEnabled)
							{
								...
								LimitSquareYZ(i != 0, ref dirX, limitYMinus, limitYPlus, limitZ, limitZ);
			...
			static bool KeepMaxLength(ref Vector3 posTo, ref Vector3 posFrom, float keepLength)
			{
			...
			static bool KeepMaxLength(ref Vector3 posTo, ref Vector3 posFrom, ref FastLength keepLength)
			{
				Vector3 v = posTo - posFrom;
				float len = SAFBIKVecLength(ref v);
			...
			static bool KeepLength(ref Vector3 posTo, ref Vector3 posFrom, float keepLength)
			{
				Vector3 v = posTo - posFrom;
				float len = SAFBIKVecLength(ref v);
				if (len > IKEpsilon)
			...
			static Quaternion GetRotation(ref Vector3 axisDir, float theta, float rate)
			{
