	...
	public partial class FullBodyIK
	{
		public class FingerIK
		{
			...
			public class FingerBranch
			{
				...
				public FingerIKParams fingerIKParams = new FingerIKParams();
			}
			...
			FingerBranch[] _fingerBranches = new FingerBranch[(int)FingerType.Max];
			ThumbBranch _thumbBranch = null;
			...
			// Allocation only.
			void PrepareBranch(int fingerType, Bone[] bones, Effector effector)
			{
				...
				FingerBranch fingerBranch = new FingerBranch();
				fingerBranch.effector = effector;

				fingerBranch.fingerLinks = new FingerLink[boneLength];
				for (int linkID = 0; linkID < boneLength; ++linkID)
				{
					...
					FingerLink fingerLink = new FingerLink();
			...
			{
				FingerBranch fingerBranch = _fingerBranches[fingerType];
				...
				if (fingerType == (int)FingerType.Thumb)
				{
					FingerBranch middleFingerBranch = _fingerBranches[(int)FingerType.Middle];
					...
					{
						FingerLink middleFingerLink0 = middleFingerBranch.fingerLinks[0];
						...
						if (fingerLinkLength >= 2 && fingerEffector._isSimulateFingerTips == false)
						{
							...
							FingerLink thumbFingerLink0 = fingerBranch.fingerLinks[fingerLinkLength - 2];
							FingerLink thumbFingerLink1 = fingerBranch.fingerLinks[fingerLinkLength - 1];
						...
						{
							FingerLink thumbFingerLink0 = fingerBranch.fingerLinks[fingerLinkLength - 3];
							FingerLink thumbFingerLink1 = fingerBranch.fingerLinks[fingerLinkLength - 2];
							FingerLink thumbFingerLink2 = fingerBranch.fingerLinks[fingerLinkLength - 1];
							Vector3 thumbPosition0 = thumbFingerLink0.bone._defaultPosition;
							Vector3 thumbPosition1 = thumbFingerLink1.bone._defaultPosition;
				...
				{
					FingerLink fingerLink = fingerBranch.fingerLinks[n];
					...
					if (fingerType == (int)FingerType.Thumb)
					{
						ThumbLink thumbLink = _thumbBranch.thumbLinks[n];
			...
			void PrepareThumb()
			{
				FingerBranch fingerBranch = _fingerBranches[(int)FingerType.Thumb];
				FingerBranch indexFingerBranch = _fingerBranches[(int)FingerType.Index];
				...
				FingerLink fingerLink0 = fingerBranch.fingerLinks[0];
				FingerLink fingerLink1 = fingerBranch.fingerLinks[1];
				FingerLink fingerLink2 = fingerBranch.fingerLinks[2];

				{
					FingerLink indexBeginLink = indexFingerBranch.fingerLinks[0];
					// Direction thumb0 to index0.
					Vector3 thumbToIndex = indexBeginLink.bone._defaultPosition - fingerLink0.bone._defaultPosition;
					SAFBIKMatMultVec(out Vector3 localThumbToIndex, ref fingerBranch.solvedToBoneBasis, ref thumbToIndex);
				...
				bool isRight)
			{
				...
				if (SAFBIKVecNormalize2(ref linkToEnd, ref linkToNext))
				{
					...
					SAFBIKComputeBasisFromXZLockX(out Matrix3x3 linkToEndBasis, dirX, rootBaseBasis.column2);
					dirX = isRight ? linkToNext : -linkToNext;
					Vector3 dirY = linkToEndBasis.column2;
					SAFBIKComputeBasisFromXZLockZ(out Matrix3x3 linkToNextBasis, ref dirX, ref dirY);
				...
				ref Vector3 effectorDirection)
			{
				...
				SAFBIKMatMultVec(out Vector3 dirZ, ref rootBasis, ref linkBoneToSolvedBasis.column2);
				if (!SAFBIKComputeBasisFromXZLockX(out Matrix3x3 linkSolvedBasis, ref dirX, ref dirZ))
				{
					return false;
				}
			...
			static float ComputeTriangleTheta(float lenA, float lenB, float lenC, float lenASq, float lenBSq, float lenCSq)
			{
			...
			public bool Solve()
			{
				...
				{
					FingerBranch fingerBranch = _fingerBranches[i];
			...
			static Vector3 GetEffectorPosition(
				...
				ref Matrix3x4 parentTransform)
			{
				if (rootBone != null && beginLinkBone != null && effector != null)
				{
					...
					if (effector.positionWeight < 1.0f - IKEpsilon)
					{
						...
						{
							endLinkPosition = effector.BoneWorldPosition;
						}
						...
						if (lengthFrom > IKEpsilon && lengthTo > IKEpsilon)
						{
							...
							Vector3 dir = LerpDir(ref dirFrom, ref dirTo, effector.positionWeight);
			...
			bool SolveNotThumb(int fingerType, ref Matrix3x4 parentTransform)
			{
				FingerBranch fingerBranch = _fingerBranches[fingerType];
				if (fingerBranch == null || fingerBranch.fingerLinks.Length != 3)
				{
				...
				FingerLink beginLink = fingerBranch.fingerLinks[0];
				FingerLink bendingLink0 = fingerBranch.fingerLinks[1];
				FingerLink bendingLink1 = fingerBranch.fingerLinks[2];
				...
				Vector3 effectorPosition = GetEffectorPosition(_internalValues, _parentBone, beginLink.bone, endEffector, fingerBranch.link0ToEffectorLength, ref parentTransform);
				...
				{
					SAFBIKMatMult(out Matrix3x3 beginToEndBasis, ref parentTransform.basis, ref fingerBranch.boneToSolvedBasis);
					SAFBIKMatMultVecInv(out Vector3 localEffectorDirection, ref beginToEndBasis, ref effectorDirection);

					isUpper = (localEffectorDirection.y >= 0.0f);

					if (LimitFingerNotThumb(
						isRight,
				...
				if (isWarp)
				{
...

					{
						...
						if (SAFBIKVecNormalize2(ref beginLinkToBendingLink0Direction, ref beginLinkToEndDirection))
						{
							...
							if (SAFBIKComputeBasisFromXZLockX(out Matrix3x3 effBasis, isRight ? effectorDirection : -effectorDirection, solveDirZ))
							{
								if (SAFBIKComputeBasisFromXZLockZ(out Matrix3x3 bendBasis, isRight ? beginLinkToBendingLink0Direction : -beginLinkToBendingLink0Direction, effBasis.column2) &&
									SAFBIKComputeBasisFromXZLockZ(out Matrix3x3 endBasis, isRight ? beginLinkToEndDirection : -beginLinkToEndDirection, effBasis.column2))
								{
									...
									Vector3 rotBendX = Rotate(ref effX, ref effY, endBendDotX, endBendDotY);
										...
										if (isWarp)
										{
											...
											if (traceLimitUAngle <= IKEpsilon || endEffDotX < cosTraceLimitUAngle)
											{
												...
												if (SAFBIKVecNormalize(ref rotBendY))
												{
													...
													beginLinkDirX = Rotate(ref rotBendX, ref rotBendY, cosTraceAngle, isRight ? -sinTraceAngle : sinTraceAngle);
												}
											}
											else
											{
												...
												beginLinkDirX = Rotate(ref bendX, ref bendY, r);
											}
										}
										else
										{
										...
										if (!solveFingerIK)
										{
											if (effectorLength < baseLength - IKEpsilon)
											{
												...
												if (extendLen > IKEpsilon && effectorLength >= baseLength - extendLen)
												{
													...
													beginLinkDirX = FastLerpDir(ref beginLinkDirX, ref effX, r);
					...
					if (!solveFingerIK)
					{
						...
						{
							if (!SAFBIKComputeBasisFromXZLockX(out Matrix3x3 effBasis, isRight ? bendingLink0ToEffectorDirection : -bendingLink0ToEffectorDirection, solveDirZ))
							{
								return false;
							}
							// Effector direction stamp X/Y Plane.(Feedback Y Axis.)
							if (!SAFBIKComputeBasisFromXZLockZ(out Matrix3x3 bendBasis, isRight ? bendingLink0ToBendingLink0Direction : -bendingLink0ToBendingLink0Direction, effBasis.column2) ||
								!SAFBIKComputeBasisFromXZLockZ(out Matrix3x3 endBasis, isRight ? bendingLink0ToEndDirection : -bendingLink0ToEndDirection, effBasis.column2))
							{
								return false;
							}
							...
							if (imm_isUpper)
							{
								bendingLink0DirX = FastLerpDir(ref rotBendX, ref effX, imm_traceRate);
							}
							else
							{
								bendingLink0DirX = FastLerpDir(ref bendX, ref effX, imm_traceRate);
							}
						}

						if (!SAFBIKComputeBasisFromXZLockX(out bendingLink0.boneTransform.basis, isRight ? Vector3.zero : -Vector3.zero, solveDirZ))
						{
							return false;
						}
...

						{
							...
							SAFBIKMatMultVec(out Vector3 dirZ, ref bendingLink0.boneTransform.basis, ref bendingLink1.boneToSolvedBasis.column2);
				...
				{
					{
						...
						if (!isUpper)
						{
							SAFBIKMatMultVec(out Vector3 dirX, ref parentTransform.basis, ref beginLink.boneToSolvedBasis.column0);

							if (SAFBIKComputeBasisFromXZLockZ(out Matrix3x3 baseBasis, dirX, solveDirZ))
							{
								SAFBIKMatMultVecInv(out Vector3 localFingerSolve, ref baseBasis, ref linkSolved);
...

					{
						...
						SAFBIKMatMultVec(out Vector3 dirZ, ref beginLink.boneTransform.basis, ref _internalValues.defaultRootBasis.column2);
				...
				SAFBIKMatMultGetRot(out Quaternion worldRotation, ref beginLink.boneTransform.basis, ref beginLink.bone._defaultBasis);
			...
			bool SolveThumb(ref Matrix3x4 parentTransform)
			{
				FingerBranch fingerBranch = _fingerBranches[(int)FingerType.Thumb];
				if (fingerBranch == null || fingerBranch.fingerLinks.Length != 3)
				{
				...
				ThumbLink thumbLink0 = _thumbBranch.thumbLinks[0];
				ThumbLink thumbLink1 = _thumbBranch.thumbLinks[1];
				ThumbLink thumbLink2 = _thumbBranch.thumbLinks[2];
...

				{
					...
					Vector3 effectorPosition = GetEffectorPosition(_internalValues, _parentBone, fingerLink0.bone, endEffector, fingerBranch.link0ToEffectorLength, ref parentTransform);
...

					{
						...
						{
							SAFBIKMatMult(out Matrix3x3 beginToEndBasis, ref parentTransform.basis, ref fingerBranch.boneToSolvedBasis);
							SAFBIKMatMultVecInv(out Vector3 localEffectorDirection, ref beginToEndBasis, ref dirX);
							if (LimitYZ(
								isRight,
								ref localEffectorDirection,
						...
						SAFBIKMatMultVec(out Vector3 dirY, ref parentTransform.basis, ref thumbLink0.thumb_boneToSolvedBasis.column1);
					...
					// thumb0 / Limit length based thumb1/2 (Type3)
					{
						...
						if (effectorLength1to3 < _thumbBranch.linkLength1to3 - IKEpsilon)
						{
							...
							float effectorLength0to3 = SAFBIKVecLengthAndLengthSq(out float effectorLength0to3Sq, ref effectorTranslate0to3);
							...
							float moveTheta = ComputeTriangleTheta(moveLenA, moveLenB, moveLenC, moveLenASq, moveLenBSq, moveLenCSq);
							if (moveTheta < baseTheta)
							{
								...
								if (newAngle > 0.01f * Mathf.Deg2Rad)
								{
									...
									{
										SAFBIKMatMultVec(out Vector3 solveDirection, ref fingerLink0.boneTransform.basis, ref _thumbBranch.thumbSolveZ);
										...
										{
											SAFBIKMatMultVec(out Vector3 dirY, ref fingerLink0.boneTransform.basis, ref fingerLink0.boneToSolvedBasis.column1);

											if (SAFBIKComputeBasisFromXYLockX(out Matrix3x3 solveBasis0, isRight ? newX : -newX, dirY))
											{
...

					{
						// thumb1
						{
							...
							SAFBIKMatMultVec(out Vector3 dirY, ref fingerLink0.boneTransform.basis, ref thumbLink1.thumb_boneToSolvedBasis.column1);
						...
						float moveThetaAtoB = ComputeTriangleTheta(moveLenA, moveLenB, moveLenC, moveLenASq, moveLenBSq, moveLenCSq);
						if (moveThetaAtoB < _thumbBranch.thumb1_baseThetaAtoB)
						{
							...
							if (newAngle > 0.01f * Mathf.Deg2Rad)
							{
								...
								{
									SAFBIKMatMultVec(out Vector3 solveDirection, ref fingerLink1.boneTransform.basis, ref _thumbBranch.thumbSolveZ);
									...
									if (SAFBIKVecNormalize(ref newX))
									{
										SAFBIKMatMultVec(out Vector3 dirY, ref fingerLink1.boneTransform.basis, ref fingerLink1.boneToSolvedBasis.column1);
										if (SAFBIKComputeBasisFromXYLockX(out Matrix3x3 solveBasis1, isRight ? newX : -newX, dirY))
										{
...

					{
						...
						SAFBIKMatMultVec(out Vector3 dirY, ref fingerLink1.boneTransform.basis, ref thumbLink2.thumb_boneToSolvedBasis.column1);
				...
				SAFBIKMatMultGetRot(out Quaternion worldRotation, ref fingerLink0.boneTransform.basis, ref fingerLink0.bone._defaultBasis);
