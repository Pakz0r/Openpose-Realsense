...
namespace SA
{
	public partial class FullBodyIK
	{
		public class LimbIK
		{
			...
			void SyncUpdateDisplacementArgs()
			{
				if (_limbIKType == LimbIKType.Leg)
				{
					float effectorMinLengthRate = _settings.limbIK.legEffectorMinLengthRate;
			...
			void SolveBaseBasis(out Matrix3x3 baseBasis, ref Matrix3x3 parentBaseBasis, ref Vector3 effectorDir)
			{
				if (_limbIKType == LimbIKType.Arm)
				{
					...
					{ // Failsafe.(Counts as default effectorDir.)
			...
			static void PrepareRollBones(ref RollBone[] rollBones, Bone[] bones)
			{
			...
			public void PresolveBeinding()
			{
				...
				SAFBIKMatSetRotMult(out Matrix3x3 parentBaseBasis, ref parentBoneWorldRotation, ref _beginBone.ParentBone._worldToBaseRotation);

				// Solve EffectorDir Based Basis.
				SolveBaseBasis(out Matrix3x3 baseBasis, ref parentBaseBasis, ref effectorDir);
			...
			bool PrefixLegEffectorPos_UpperNear(ref Vector3 localEffectorTrans)
			{
			...
			static bool PrefixLegEffectorPos_Circular_Far(ref Vector3 localEffectorTrans, float effectorLength)
			{
				return PrefixLegEffectorPos_Circular(ref localEffectorTrans, effectorLength, true);
			}

			static bool PrefixLegEffectorPos_Circular(ref Vector3 localEffectorTrans, float effectorLength, bool isFar)
			{
			...
			static bool PrefixLegEffectorPos_Upper_Circular_Far(ref Vector3 localEffectorTrans,
			...
			// Lefthand based.
			static void ComputeLocalDirXZ(ref Vector3 localDir, out Vector3 localDirXZ)
			{
			...
			// Lefthand based.
			static void ComputeLocalDirYZ(ref Vector3 localDir, out Vector3 localDirYZ)
			{
			...
			{
				return PresolveInternal(ref parentBaseBasis, ref beginPos, out float effectorLen, out Matrix3x3 baseBasis, out solvedBeginToBendingDir, out solvedBendingToEndDir);
			}
				...
				out Vector3 solvedBendingToEndDir)
			{
				...
				{ // Override Effector Pos.
					SAFBIKMatMultVec(out Vector3 localEffectorTrans, ref parentBaseBasisInv, ref effectorTrans);
					...
					{ // Front
						...
						if (!isProcessed &&
							localEffectorTrans.y >= -_effectorMinLength.value &&
							localEffectorTrans.z <= _leg_upperLimitNearCircleZ)
						{ // Upper(Near)
							isProcessed = true;
				...
				if (!_bendingEffector.positionEnabled)
				{
					...
					if (presolvedEnabled && _isPresolvedBending)
					{
						...
						if (presolvedBendingRate > IKEpsilon)
						{
							...
							if (_presolvedLerpTheta.cos < 1.0f - IKEpsilon)
							{ // Lerp
						...
						{
							Vector3 bendingDir;
							SAFBIKMatMult(out Matrix3x3 presolvedBendingBasis, ref baseBasis, ref _presolvedBendingBasis);
						...
						if (_limbIKType == LimbIKType.Arm)
						{
							Vector3 dirX = (_limbIKSide == Side.Left) ? -baseBasis.column0 : baseBasis.column0;
							{
								...
								if (_settings.limbIK.armEffectorBackfixEnabled)
								{
									...
									ComputeLocalDirXZ(ref localDir, out Vector3 localXZ); // Lefthand Based. X is reversed in RightSide
									ComputeLocalDirYZ(ref localDir, out Vector3 localYZ); // Lefthand Based.
								...
								if (!SAFBIKVecNormalize2(ref dirY, ref dirZ))
								{ // Failsafe.
				...
				if (isSolved && _limbIKType == LimbIKType.Arm && _settings.limbIK.armEffectorInnerfixEnabled)
				{
					...
					SAFBIKMatMultVec(out Vector3 localBendingDir, ref parentBaseBasisInv, ref solvedBeginToBendingDir);
					...
					if (localX > limitTheta)
					{
						...
						SAFBIKMatMultVec(out Vector3 bendingDir, ref parentBaseBasis, ref localBendingDir);
			...
			public bool SolveInternal()
			{
				...
				if (!PresolveInternal(ref parentBaseBasis, ref beginPos, out float effectorLen, out Matrix3x3 baseBasis, out Vector3 solvedBeginToBendingDir, out Vector3 solvedBendingToEndDir))
				{
					return false;
				}
						...
						else
						{
							...
							if (forcefixEnabled && effectorLen > _arm_elbowBasisForcefixEffectorLengthBegin.value)
							{
								...
								if (t > IKEpsilon)
								{
									...
									SAFBIKMatMultCol1(out Vector3 tempY, ref beginBasis, ref _beginToBendingBoneBasis);
				...
				if (_limbIKType == LimbIKType.Arm)
				{
					...
					_arm_solvedBendingBoneBasis = Matrix3x3.identity;
				}

				SAFBIKMatMultGetRot(out Quaternion worldRotation, ref beginBasis, ref _beginBone._boneToWorldBasis);
			...
			bool SolveEndRotation(bool isSolved, ref Quaternion bendingBonePrevRotation, ref Quaternion endBonePrevRotation)
			{
				float endRotationWeight = _endEffector.rotationEnabled ? _endEffector.rotationWeight : 0.0f;
				if (endRotationWeight > IKEpsilon)
				{
					Quaternion endEffectorWorldRotation = _endEffector.WorldRotation;
					SAFBIKQuatMult(out Quaternion toRotation, ref endEffectorWorldRotation, ref _endEffectorToWorldRotation);
			...
			bool RollInternal()
			{
				...
				if (_armRollBones != null && _armRollBones.Length > 0)
				{
					...
					if (SAFBIKComputeBasisLockX(out Matrix3x3 bendingBasisTo, ref dirX, ref dirY, ref dirZ))
					{
						SAFBIKMatMult(out Matrix3x3 baseBasis, ref beginBoneBasis, ref _beginBone._boneToBaseBasis);
						SAFBIKMatMult(out Matrix3x3 baseBasisTo, ref bendingBasisTo, ref _beginBone._boneToBaseBasis);
						...
						for (int i = 0; i < boneLength; ++i)
						{
							if (_armRollBones[i].bone != null && _armRollBones[i].bone.TransformIsAlive)
							{
								...
								SAFBIKMatFastLerp(out Matrix3x3 tempBasis, ref baseBasis, ref baseBasisTo, rate);
								SAFBIKMatMultGetRot(out Quaternion worldRotation, ref tempBasis, ref _elbowRollBones[i].bone._baseToWorldBasis);
				...
				if (_elbowRollBones != null && _elbowRollBones.Length > 0)
				{
					...
					if (SAFBIKVecNormalize2(ref dirY, ref dirZ))
					{ // Lock dirX(bendingBoneBasis.column0)
						Matrix3x3 baseBasisTo = Matrix3x3.FromColumn(ref dirX, ref dirY, ref dirZ);
						SAFBIKMatMult(out Matrix3x3 baseBasis, ref bendingBoneBasis, ref _bendingBone._boneToBaseBasis);
