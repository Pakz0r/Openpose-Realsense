	...
	public static class FullBodyIKEditorUtility
	{
		...
		public static void SetDirty(UnityEngine.Object target)
		{
			if (target != null)
			{
				...
				{
					SetDirty((Component)(target as Transform));
				}
				else if (target is GameObject)
				{
					SetDirty(target as GameObject);
				}
				else if (target is Component)
				{
					SetDirty(target as Component);
				}
				else
				{
					EditorUtility.SetDirty(target);
		...
		static void SetDirty(UnityEngine.Component target)
		{
			if (target != null)
			{
				if (IsInSceneGameObject(target.gameObject))
				{
		...
		static void SetDirty(UnityEngine.GameObject target)
		{
			if (target != null)
			{
				if (IsInSceneGameObject(target))
				{
					if (EditorApplication.isPlaying)
					{
						return;
		...
		static bool IsInSceneGameObject(GameObject gameObject)
		{
			if (gameObject == null)
			{
				return false;
