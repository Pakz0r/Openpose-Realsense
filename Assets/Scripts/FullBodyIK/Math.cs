...
namespace SA
{
	public partial class FullBodyIK
	{
		...
		public struct Matrix3x4
		{
			...
			public Matrix3x4 Inverse
			{
				get
				{
					...
					SAFBIKMatMultVec(out Vector3 tmp, ref basis_inv, ref origin_minus);
			...
			{
				SAFBIKMatMultVecAdd(out Vector3 tmp, ref basis, ref v, ref origin);
				return tmp;
			}
			...
			{
				SAFBIKMatMultVecAdd(out Vector3 tmp, ref t.basis, ref v, ref t.origin);
				return tmp;
			}

			public Matrix3x4 Multiply(Matrix3x4 t)
			{
				SAFBIKMatMult(out Matrix3x3 tmp_basis, ref basis, ref t.basis);
				SAFBIKMatMultVecAdd(out Vector3 tmp_origin, ref basis, ref t.origin, ref origin);
			...
			{
				SAFBIKMatMult(out Matrix3x3 tmp_basis, ref t1.basis, ref t2.basis);
				SAFBIKMatMultVecAdd(out Vector3 tmp_origin, ref t1.basis, ref t2.origin, ref t1.origin);
				return new Matrix3x4(ref tmp_basis, ref tmp_origin);
			}
		...
		{
			SAFBIKMatMult(out Matrix3x3 tmpMat, ref lhs, ref rhs);
		...
		{
			bool solveY = SAFBIKComputeBasisFromXYLockX(out Matrix3x3 basisY, ref dirX, ref dirY);
			bool solveZ = SAFBIKComputeBasisFromXZLockX(out Matrix3x3 basisZ, ref dirX, ref dirZ);
			if (solveY && solveZ)
			{
		...
		{
			bool solveX = SAFBIKComputeBasisFromXYLockY(out Matrix3x3 basisX, ref dirX, ref dirY);
			bool solveZ = SAFBIKComputeBasisFromYZLockY(out Matrix3x3 basisZ, ref dirY, ref dirZ);
			if (solveX && solveZ)
			{
		...
		{
			bool solveX = SAFBIKComputeBasisFromXZLockZ(out Matrix3x3 basisX, ref dirX, ref dirZ);
			bool solveY = SAFBIKComputeBasisFromYZLockZ(out Matrix3x3 basisY, ref dirY, ref dirZ);
			if (solveX && solveY)
			{
		...
		public static bool IsNear(ref Vector3 lhs, ref Vector3 rhs)
		{
		...
		public static Vector3 Rotate(ref Vector3 dirX, ref Vector3 dirY, float cosR, float sinR)
		{
			return dirX * cosR + dirY * sinR;
		}

		public static Vector3 Rotate(ref Vector3 dirX, ref Vector3 dirY, float r)
		{
		...
		public static Vector3 Rotate(ref Vector3 dirX, ref Vector3 dirY, ref FastAngle angle)
		{
			return dirX * angle.cos + dirY * angle.sin;
		}

		public static bool NormalizedTranslate(ref Vector3 dir, ref Vector3 fr, ref Vector3 to)
		{
			Vector3 t = to - fr;
		...
		public struct FastLength
		{
			...
			{
				float length = SAFBIKVecLengthAndLengthSq(out float lengthSq, ref v);
				return new FastLength(length, lengthSq);
			}

			public static FastLength FromVector3(Vector3 v0, Vector3 v1)
			{
				float length = SAFBIKVecLengthAndLengthSq2(out float lengthSq, ref v0, ref v1);
				return new FastLength(length, lengthSq);
			}
		...
		public static bool SAFBIKComputeBasisFrom(out Matrix3x3 basis, ref Matrix3x3 rootBasis, ref Vector3 dir, DirectionAs directionAs)
		{
			CheckNormalized(dir);
		...
		// Limited Square.
		public static bool LimitSquareXY(
		...
		public static bool LimitSquareYZ(
			bool isRight,
		...
		// Limited Square.
		public static bool LimitSquareXZ(
			ref Vector3 dir,                // dirZ
			float limitXMinus,               // X-
			float limitXPlus,                // X+
			float limitZMinus,               // Z-
		...
		public static Vector3 FitToPlane(Vector3 pos, Vector3 planeDir)
		{
		...
		public static bool FitToPlaneDir(ref Vector3 dir, Vector3 planeDir)
		{
		...
		public static bool LimitToPlaneDirY(ref Vector3 dir, Vector3 planeDir, float thetaY)
		{
			float d = Vector3.Dot(dir, planeDir);
			if (d <= IKEpsilon && d >= -IKEpsilon)
			{
		...
		static void LerpRotateBasis(out Matrix3x3 basis, ref Vector3 axis, float cos, float rate)
		{
			if (rate <= IKEpsilon)
			{
				basis = Matrix3x3.identity;
		...
		public static Vector3 LerpDir(ref Vector3 src, ref Vector3 dst, float r)
		{
			if (ComputeThetaAxis(ref src, ref dst, out float theta, out Vector3 axis))
			{
				LerpRotateBasis(out Matrix3x3 basis, ref axis, theta, r);
				SAFBIKMatMultVec(out Vector3 tmp, ref basis, ref src);
				return tmp;
			}
		...
		public static Vector3 FastLerpDir(ref Vector3 src, ref Vector3 dst, float r)
		{
			if (r <= IKEpsilon)
			{
				return src;
