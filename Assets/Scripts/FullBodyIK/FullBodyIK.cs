...
namespace SA
{
	...
	public partial class FullBodyIK
	{
		...
		public class InternalValues
		{
			...
			public class BodyIK
			{
				...
				public void Update(Settings.BodyIK settingsBodyIK)
				{
					...
					{
						spineLimitAngleX.Reset(settingsBodyIK.spineLimitAngleX, Mathf.Deg2Rad);
					}
					if (spineLimitAngleY._a != settingsBodyIK.spineLimitAngleY)
					{
						spineLimitAngleY.Reset(settingsBodyIK.spineLimitAngleY, Mathf.Deg2Rad);
					}
		...
		public class BoneCaches
		{
			...
			Vector3 GetHipsOffset(int index, Bone legBone, Bone kneeBone, Bone footBone)
			{
				if (legBone != null && kneeBone != null && footBone != null)
				{
					float hipsToLegLen = legBone._defaultLocalLength.length;
			...
			public void SyncDisplacement(FullBodyIK fullBodyIK)
			{
				Assert(fullBodyIK != null);

				Vector3 hipsOffset0 = GetHipsOffset(0, fullBodyIK.leftLegBones.leg, fullBodyIK.leftLegBones.knee, fullBodyIK.leftLegBones.foot);
				Vector3 hipsOffset1 = GetHipsOffset(1, fullBodyIK.rightLegBones.leg, fullBodyIK.rightLegBones.knee, fullBodyIK.rightLegBones.foot);
		...
		static void SetBoneTransform(ref Bone bone, Transform transform)
		{
			if (bone == null)
			{
				bone = new Bone();
		...
		static void SetFingerBoneTransform(ref Bone[] bones, Transform[,] transforms, int index)
		{
		...
		static bool IsSpine(Transform trn)
		{
			if (trn != null)
			{
				string name = trn.name;

				if (name.Contains("Spine", System.StringComparison.InvariantCultureIgnoreCase))
				{
					return true;
				}
				if (name.Contains("Torso", System.StringComparison.InvariantCultureIgnoreCase))
				{
					return true;
				}
			}
		...
		static bool IsNeck(Transform trn)
		{
			if (trn != null)
			{
				string name = trn.name;
				if (name != null)
				{
					if (name.Contains("Neck", System.StringComparison.InvariantCultureIgnoreCase))
					{
						return true;
					}
					if (name.Contains("Kubi", System.StringComparison.InvariantCultureIgnoreCase))
					{
						return true;
					}
					if (name.Contains("\u304F\u3073"))
		...
		void Prefix()
		{
			...
			Prefix(ref bodyBones.hips, BoneLocation.Hips, null);
			Prefix(ref bodyBones.spine, BoneLocation.Spine, bodyBones.hips);
			Prefix(ref bodyBones.spine2, BoneLocation.Spine2, bodyBones.spine);
			Prefix(ref bodyBones.spine3, BoneLocation.Spine3, bodyBones.spine2);
			Prefix(ref bodyBones.spine4, BoneLocation.Spine4, bodyBones.spine3);
			Prefix(ref headBones.neck, BoneLocation.Neck, bodyBones.SpineU);
			Prefix(ref headBones.head, BoneLocation.Head, headBones.neck);
			Prefix(ref headBones.leftEye, BoneLocation.LeftEye, headBones.head);
			Prefix(ref headBones.rightEye, BoneLocation.RightEye, headBones.head);
			for (int i = 0; i != 2; ++i)
			{
				var legBones = (i == 0) ? leftLegBones : rightLegBones;
				Prefix(ref legBones.leg, (i == 0) ? BoneLocation.LeftLeg : BoneLocation.RightLeg, bodyBones.hips);
				Prefix(ref legBones.knee, (i == 0) ? BoneLocation.LeftKnee : BoneLocation.RightKnee, legBones.leg);
				Prefix(ref legBones.foot, (i == 0) ? BoneLocation.LeftFoot : BoneLocation.RightFoot, legBones.knee);

				var armBones = (i == 0) ? leftArmBones : rightArmBones;
				Prefix(ref armBones.shoulder, (i == 0) ? BoneLocation.LeftShoulder : BoneLocation.RightShoulder, bodyBones.SpineU);
				Prefix(ref armBones.arm, (i == 0) ? BoneLocation.LeftArm : BoneLocation.RightArm, armBones.shoulder);
				Prefix(ref armBones.elbow, (i == 0) ? BoneLocation.LeftElbow : BoneLocation.RightElbow, armBones.arm);
				Prefix(ref armBones.wrist, (i == 0) ? BoneLocation.LeftWrist : BoneLocation.RightWrist, armBones.elbow);

				for (int n = 0; n != MaxArmRollLength; ++n)
				{
					var armRollLocation = (i == 0) ? BoneLocation.LeftArmRoll0 : BoneLocation.RightArmRoll0;
					Prefix(ref armBones.armRoll[n], (BoneLocation)((int)armRollLocation + n), armBones.arm);
				}

				for (int n = 0; n != MaxElbowRollLength; ++n)
				{
					var elbowRollLocation = (i == 0) ? BoneLocation.LeftElbowRoll0 : BoneLocation.RightElbowRoll0;
					Prefix(ref armBones.elbowRoll[n], (BoneLocation)((int)elbowRollLocation + n), armBones.elbow);
				}
				...
				for (int n = 0; n != MaxHandFingerLength; ++n)
				{
					...
					Prefix(ref fingerBones.thumb[n], (BoneLocation)((int)thumbLocation + n), (n == 0) ? armBones.wrist : fingerBones.thumb[n - 1]);
					Prefix(ref fingerBones.index[n], (BoneLocation)((int)indexLocation + n), (n == 0) ? armBones.wrist : fingerBones.index[n - 1]);
					Prefix(ref fingerBones.middle[n], (BoneLocation)((int)middleLocation + n), (n == 0) ? armBones.wrist : fingerBones.middle[n - 1]);
					Prefix(ref fingerBones.ring[n], (BoneLocation)((int)ringLocation + n), (n == 0) ? armBones.wrist : fingerBones.ring[n - 1]);
					Prefix(ref fingerBones.little[n], (BoneLocation)((int)littleLocation + n), (n == 0) ? armBones.wrist : fingerBones.little[n - 1]);
				}
			}

			Prefix(ref rootEffector, EffectorLocation.Root);
			Prefix(ref bodyEffectors.hips, EffectorLocation.Hips, rootEffector, bodyBones.hips, leftLegBones.leg, rightLegBones.leg);
			Prefix(ref headEffectors.neck, EffectorLocation.Neck, bodyEffectors.hips, headBones.neck);
			Prefix(ref headEffectors.head, EffectorLocation.Head, headEffectors.neck, headBones.head);
			Prefix(ref headEffectors.eyes, EffectorLocation.Eyes, rootEffector, headBones.head, headBones.leftEye, headBones.rightEye);

			Prefix(ref leftLegEffectors.knee, EffectorLocation.LeftKnee, rootEffector, leftLegBones.knee);
			Prefix(ref leftLegEffectors.foot, EffectorLocation.LeftFoot, rootEffector, leftLegBones.foot);
			Prefix(ref rightLegEffectors.knee, EffectorLocation.RightKnee, rootEffector, rightLegBones.knee);
			Prefix(ref rightLegEffectors.foot, EffectorLocation.RightFoot, rootEffector, rightLegBones.foot);

			Prefix(ref leftArmEffectors.arm, EffectorLocation.LeftArm, bodyEffectors.hips, leftArmBones.arm);
			Prefix(ref leftArmEffectors.elbow, EffectorLocation.LeftElbow, bodyEffectors.hips, leftArmBones.elbow);
			Prefix(ref leftArmEffectors.wrist, EffectorLocation.LeftWrist, bodyEffectors.hips, leftArmBones.wrist);
			Prefix(ref rightArmEffectors.arm, EffectorLocation.RightArm, bodyEffectors.hips, rightArmBones.arm);
			Prefix(ref rightArmEffectors.elbow, EffectorLocation.RightElbow, bodyEffectors.hips, rightArmBones.elbow);
			Prefix(ref rightArmEffectors.wrist, EffectorLocation.RightWrist, bodyEffectors.hips, rightArmBones.wrist);

			Prefix(ref leftHandFingersEffectors.thumb, EffectorLocation.LeftHandThumb, leftArmEffectors.wrist, leftHandFingersBones.thumb);
			Prefix(ref leftHandFingersEffectors.index, EffectorLocation.LeftHandIndex, leftArmEffectors.wrist, leftHandFingersBones.index);
			Prefix(ref leftHandFingersEffectors.middle, EffectorLocation.LeftHandMiddle, leftArmEffectors.wrist, leftHandFingersBones.middle);
			Prefix(ref leftHandFingersEffectors.ring, EffectorLocation.LeftHandRing, leftArmEffectors.wrist, leftHandFingersBones.ring);
			Prefix(ref leftHandFingersEffectors.little, EffectorLocation.LeftHandLittle, leftArmEffectors.wrist, leftHandFingersBones.little);

			Prefix(ref rightHandFingersEffectors.thumb, EffectorLocation.RightHandThumb, rightArmEffectors.wrist, rightHandFingersBones.thumb);
			Prefix(ref rightHandFingersEffectors.index, EffectorLocation.RightHandIndex, rightArmEffectors.wrist, rightHandFingersBones.index);
			Prefix(ref rightHandFingersEffectors.middle, EffectorLocation.RightHandMiddle, rightArmEffectors.wrist, rightHandFingersBones.middle);
			Prefix(ref rightHandFingersEffectors.ring, EffectorLocation.RightHandRing, rightArmEffectors.wrist, rightHandFingersBones.ring);
			Prefix(ref rightHandFingersEffectors.little, EffectorLocation.RightHandLittle, rightArmEffectors.wrist, rightHandFingersBones.little);
		...
		static Transform FindEye(Transform head, bool isRight)
		{
			if (head != null)
			{
		...
		public void ConfigureBoneTransforms()
		{
			...
			if (settings.automaticPrepareHumanoid && rootTransform != null)
			{
				...
				if (animator != null && animator.isHuman)
				{
					...
					if (leftEye == null)
					{
						leftEye = FindEye(head, false);
					}
					if (rightEye == null)
					{
						rightEye = FindEye(head, true);
					}
					...
					SetBoneTransform(ref bodyBones.hips, hips);
					SetBoneTransform(ref bodyBones.spine, spine);
					SetBoneTransform(ref bodyBones.spine2, spine2);
					SetBoneTransform(ref bodyBones.spine3, spine3);
					SetBoneTransform(ref bodyBones.spine4, spine4);

					SetBoneTransform(ref headBones.neck, neck);
					SetBoneTransform(ref headBones.head, head);
					SetBoneTransform(ref headBones.leftEye, leftEye);
					SetBoneTransform(ref headBones.rightEye, rightEye);

					SetBoneTransform(ref leftLegBones.leg, leftLeg);
					SetBoneTransform(ref leftLegBones.knee, leftKnee);
					SetBoneTransform(ref leftLegBones.foot, leftFoot);
					SetBoneTransform(ref rightLegBones.leg, rightLeg);
					SetBoneTransform(ref rightLegBones.knee, rightKnee);
					SetBoneTransform(ref rightLegBones.foot, rightFoot);

					SetBoneTransform(ref leftArmBones.shoulder, leftShoulder);
					SetBoneTransform(ref leftArmBones.arm, leftArm);
					SetBoneTransform(ref leftArmBones.elbow, leftElbow);
					SetBoneTransform(ref leftArmBones.wrist, leftWrist);
					SetBoneTransform(ref rightArmBones.shoulder, rightShoulder);
					SetBoneTransform(ref rightArmBones.arm, rightArm);
					SetBoneTransform(ref rightArmBones.elbow, rightElbow);
					SetBoneTransform(ref rightArmBones.wrist, rightWrist);

					SetFingerBoneTransform(ref leftHandFingersBones.thumb, leftFingers, 0);
					SetFingerBoneTransform(ref leftHandFingersBones.index, leftFingers, 1);
					SetFingerBoneTransform(ref leftHandFingersBones.middle, leftFingers, 2);
					SetFingerBoneTransform(ref leftHandFingersBones.ring, leftFingers, 3);
					SetFingerBoneTransform(ref leftHandFingersBones.little, leftFingers, 4);

					SetFingerBoneTransform(ref rightHandFingersBones.thumb, rightFingers, 0);
					SetFingerBoneTransform(ref rightHandFingersBones.index, rightFingers, 1);
					SetFingerBoneTransform(ref rightHandFingersBones.middle, rightFingers, 2);
					SetFingerBoneTransform(ref rightHandFingersBones.ring, rightFingers, 3);
					SetFingerBoneTransform(ref rightHandFingersBones.little, rightFingers, 4);
				}
			}
			...
			if (settings.automaticConfigureRollBonesEnabled)
			{
				...
				for (int side = 0; side != 2; ++side)
				{
						...
						armBones.wrist != null && armBones.wrist.transform != null)
					{
						...
						ConfigureRollBones(armBones.armRoll, tempBones, armBones.arm.transform, armBones.elbow.transform, (Side)side, true);
						ConfigureRollBones(armBones.elbowRoll, tempBones, armBones.elbow.transform, armBones.wrist.transform, (Side)side, false);
					}
				}
			}
		}

		void ConfigureRollBones(Bone[] bones, List<Transform> tempBones, Transform transform, Transform excludeTransform, Side side, bool isArm)
		{
			...
			for (int i = 0; i != childCount; ++i)
			{
				SetBoneTransform(ref bones[i], tempBones[i]);
			}
		}
		...
		public void Update()
		{
			...
			if (_limbIK != null || _headIK != null)
			{
                ...
                {
                    leftArmBones.shoulder?.ForcefixWorldRotation();
					rightArmBones.shoulder?.ForcefixWorldRotation();
				}
		...
		public void DrawGizmos()
		{
			...
			if (_effectors != null)
			{
				...
				for (int i = 0; i != effectorLength; ++i)
				{
					DrawEffectorGizmo(_effectors[i]);
				}
			}
			...
			if (_bones != null)
			{
				...
				for (int i = 0; i != boneLength; ++i)
				{
					DrawBoneGizmo(_bones[i], ref cameraForward);
				}
			}
		...
		static void DrawEffectorGizmo(Effector effector)
		{
			if (effector != null)
			{
				...
				Gizmos.DrawWireSphere(Vector3.zero, effectorSize);
				Gizmos.DrawWireSphere(Vector3.zero, effectorSize);
				Gizmos.DrawWireSphere(Vector3.zero, effectorSize);
				Gizmos.DrawWireSphere(Vector3.zero, effectorSize);
			}
		...
		void DrawBoneGizmo(Bone bone, ref Vector3 cameraForward)
		{
            ...
            SAFBIKMatSetRot(out Matrix3x3 basis, ref rotation);

            DrawTransformGizmo(position, ref basis, ref cameraForward, bone.BoneType);
			...
			if (parentTransform != null)
			{
				...
				DrawBoneGizmo(parentTransform.position, position, ref basis, ref cameraForward, bone.BoneType);
			}
		}

		static void DrawTransformGizmo(Vector3 position, ref Matrix3x3 basis, ref Vector3 cameraForward, BoneType boneType)
		{
			...
			// X Axis
			DrawArrowGizmo(Color.red, ref position, ref column0, ref cameraForward, boneType);
			// Y Axis
			DrawArrowGizmo(Color.green, ref position, ref column1, ref cameraForward, boneType);
			// Z Axis
			DrawArrowGizmo(Color.blue, ref position, ref column2, ref cameraForward, boneType);
		}
		...
		static void DrawBoneGizmo(Vector3 fromPosition, Vector3 toPosition, ref Matrix3x3 basis, ref Vector3 cameraForward, BoneType boneType)
		{
		...
		void Prefix(ref Bone bone, BoneLocation boneLocation, Bone parentBoneLocationBased)
		{
			Assert(_bones != null);
		...
		void Prefix(
			...
			Bone[] bones)
		{
			Prefix(ref effector, effectorLocation, parentEffector, (bones != null && bones.Length > 0) ? bones[bones.Length - 1] : null);
		}
		...
		// Custom Solver.
		public virtual bool IsHiddenCustomEyes()
		{
			return false;
		}

		public virtual bool PrepareCustomEyes(ref Quaternion headToLeftEyeRotation, ref Quaternion headToRightEyeRotation)
		{
			return false;
		}

		public virtual void ResetCustomEyes()
		{
		}

		public virtual void SolveCustomEyes(ref Matrix3x3 neckBasis, ref Matrix3x3 headBasis, ref Matrix3x3 headBaseBasis)
		{
		}
	}
}